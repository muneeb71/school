...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <form class="" method="post" action="{{ route('subjects.update', $s->id) }}">
                        @csrf @method('PUT')
                        ...
                        <div class="form-group row">
                                    <label for="name" class="col-lg-3 col-form-label font-weight-semibold">Section <span class="text-danger">*</span></label>
                                    <div class="col-lg-9">
                                        <input id="section" name="section" value="{{ $s->section }}" required type="text" class="form-control" placeholder="Section">
                                    </div>
                                </div>
                        <div class="form-group row">
                            <label for="combination" class="col-lg-3 col-form-label font-weight-semibold">Combination <span class="text-danger">*</span></label>
                            <div class="col-lg-9">
                                <select required data-placeholder="Select Combination" class="form-control select" name="combination" id="combination">
                                    @foreach($combination as $c)
                                        <option {{ $s->combination == $c->name ? 'selected' : '' }} value="{{ $c->name }}">{{ $c->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
